...
export type CallStatus =
  ...
  | "busy" | "failed" | "no-answer" | "canceled" | string;
...
export const DialerProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [apiBase, setApiBase] = useState(localStorage.getItem("apiBase") || envApi || "");
  useEffect(() => { localStorage.setItem("apiBase", apiBase); }, [apiBase]);

  const envPool = getEnv("VITE_FROM_POOL") || "";
  const [fromPoolRaw, setFromPoolRaw] = useState(localStorage.getItem("fromPool") || envPool);
  useEffect(() => { localStorage.setItem("fromPool", fromPoolRaw); }, [fromPoolRaw]);

  const fromPool = useMemo(
    () => fromPoolRaw.split(",").map(s => s.trim()).filter(Boolean),
    [fromPoolRaw]
  );
  ...
  const addHistory = (e: HistoryEntry) => setHistory(h => [e, ...h].slice(0, 500));
